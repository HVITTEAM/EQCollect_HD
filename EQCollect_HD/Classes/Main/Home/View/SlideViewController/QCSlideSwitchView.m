...
- (void)initValues
{
    ...
    //创建主滚动视图
    _rootScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, kHeightOfTopScrollView, self.width, self.bounds.size.height - kHeightOfTopScrollView)];
...
- (void)layoutSubviews
{
    ...
    if (_isBuildUI) {
        ...
        {
            _rigthSideButton.frame = CGRectMake(self.width - self.rigthSideButton.bounds.size.width, 0,
        ...
        for (int i = 0; i < [_viewArray count]; i++)
        {
            ...
            listVC.view.frame = CGRectMake(0+self.width*i, 0,
                                           self.width, self.height - kHeightOfTopScrollView);
        }
...
- (void)createNameButtons
{
    ...
    if ([self getAllBtnTitleWidth] < self.width)
...
{
    CGFloat btnWidth = self.width/_viewArray.count;
    ...
    //设置顶部滚动视图的内容总尺寸
    _topScrollView.contentSize = CGSizeMake(self.width, kHeightOfTopScrollView);
}

/**
 *  横竖屏切换时需要重新布局tabButton
 *  @author dxw
 */
-(void)adjustTabButtonFrame
{
    if ([self getAllBtnTitleWidth] <  self.width)
    {
        CGFloat btnWidth = self.width/_viewArray.count;
        for (int i = 0; i < [_viewArray count]; i++)
        {
            ...
            [button setFrame:CGRectMake(btnWidth*i,0,btnWidth, kHeightOfTopScrollView)];
            if (_userSelectedChannelID == i+100) {
                [UIView animateWithDuration:0.25 animations:^{
                    [_shadowImageView setFrame:CGRectMake(button.frame.origin.x, 0, button.frame.size.width, _shadowImage.size.height)];
    ...
    if (!sender.selected) {
        ...
        [UIView animateWithDuration:0.25 animations:^{
            ...
            if (finished) {
                //设置新页出现
                if (!_isRootScroll) {
                    [_rootScrollView setContentOffset:CGPointMake((sender.tag - 100)*self.width, 0) animated:YES];
                }
                _isRootScroll = NO;
...
{
    if (sender.frame.origin.x - _topScrollView.contentOffset.x > self.width - (kWidthOfButtonMargin+sender.bounds.size.width)) {
        //向左滚动视图，显示完整tab文字
