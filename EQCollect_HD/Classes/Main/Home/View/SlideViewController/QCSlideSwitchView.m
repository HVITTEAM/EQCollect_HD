...
#define slideWithchViewW self.width
#define slideWithchViewH self.height
...
{
    //创建顶部可滑动的tab1
    _topScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, slideWithchViewW, kHeightOfTopScrollView)];
 ...
 //   [_topScrollView addSubview:bgview];
    ...
    //创建主滚动视图
    _rootScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, kHeightOfTopScrollView, slideWithchViewW, self.bounds.size.height - kHeightOfTopScrollView)];
...
- (void)layoutSubviews
{
    ...
    if (_isBuildUI) {
        ...
        if (self.rigthSideButton.bounds.size.width > 0) {
            _rigthSideButton.frame = CGRectMake(slideWithchViewW - self.rigthSideButton.bounds.size.width, 0,
...
//                                              MTScreenW - self.rigthSideButton.bounds.size.width, kHeightOfTopScrollView);
        ...
        //更新主视图的总宽度
        _rootScrollView.contentSize = CGSizeMake(slideWithchViewW * [_viewArray count], 0);
        
        //更新主视图各个子视图的宽度
        for (int i = 0; i < [_viewArray count]; i++) {
            UIViewController *listVC = _viewArray[i];
            listVC.view.frame = CGRectMake(0+slideWithchViewW*i, 0,
                                           slideWithchViewW, slideWithchViewH-kHeightOfTopScrollView);
        }
        
        //滚动到选中的视图
        [_rootScrollView setContentOffset:CGPointMake((_userSelectedChannelID - 100)*slideWithchViewW, 0) animated:NO];
...
- (void)createNameButtons
{
    ...
    //调用方法判断button的总宽度，大于QCSlideSwitchView的宽度按button标题长度来设置，否则平均分配QCSlideSwitchView的宽度
    if ([self getAllBtnTitleWidth] < slideWithchViewW) {
        [self createNameButtonsByWidth];
        return;
    }
    [self createNameButtonsByButtonTitle];
}

/**
 *  计算所有button占据的总宽度
 */
-(CGFloat)getAllBtnTitleWidth
{
    CGFloat allBtnTitleWidth = kWidthOfButtonMargin;
    for (int i = 0; i < [_viewArray count]; i++) {
        ...
        //累计每个tab文字的长度
        allBtnTitleWidth += kWidthOfButtonMargin+textSize.width;
    }
    return allBtnTitleWidth;
}

/**
 *  根据button的标题初始化顶部tab的各个按钮
 */
-(void)createNameButtonsByButtonTitle
{
    //顶部tabbar的总长度
...
/**
 *  根据QCSlideSwitchViewr的宽初始化顶部tab的各个按钮
 */
- (void)createNameButtonsByWidth
{
    CGFloat btnWidth = slideWithchViewW/_viewArray.count;
    ...
    for (int i = 0; i < [_viewArray count]; i++) {
        ...
        [button setTag:i+100];
        if (i == 0) {
            _shadowImageView.frame = CGRectMake(0, 0, btnWidth, _shadowImage.size.height);
            button.selected = YES;
        }
        [button setTitle:vc.title forState:UIControlStateNormal];
         button.titleLabel.font = [UIFont systemFontOfSize:kFontSizeOfTabButton];
    ...
    //设置顶部滚动视图的内容总尺寸
    _topScrollView.contentSize = CGSizeMake(slideWithchViewW, kHeightOfTopScrollView);
}
    ...
    if (!sender.selected) {
        ...
        [UIView animateWithDuration:0.25 animations:^{
            ...
            if (finished) {
                //设置新页出现
                if (!_isRootScroll) {
                    [_rootScrollView setContentOffset:CGPointMake((sender.tag - 100)*slideWithchViewW, 0) animated:YES];
                }
                _isRootScroll = NO;
...
{
    if (sender.frame.origin.x - _topScrollView.contentOffset.x > slideWithchViewW - (kWidthOfButtonMargin+sender.bounds.size.width)) {
        //向左滚动视图，显示完整tab文字
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    if (scrollView == _rootScrollView) {
        _isRootScroll = YES;
        //调整顶部滑条按钮状态
        int tag = (int)(scrollView.contentOffset.x/slideWithchViewW) +100;
